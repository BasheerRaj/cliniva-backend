...
export class UserAccessService {
  ...
  async createUserAccess(
    ...
    await this.validateScopeEntity(
      createAccessDto.scopeType,
      createAccessDto.scopeId,
    );
    ...
    const accessData = {
      ...
      expiresAt: createAccessDto.expiresAt
        ? new Date(createAccessDto.expiresAt)
        : undefined,
    ...
    this.logger.log(
      `User access created successfully for user: ${createAccessDto.userId}`,
    );
    return savedAccess;
  }
  ...
  async updateUserAccess(
    ...
    const updateData: any = {
      ...updateAccessDto,
      expiresAt: updateAccessDto.expiresAt
        ? new Date(updateAccessDto.expiresAt)
        : undefined,
  ...
  async checkPermission(checkDto: CheckPermissionDto): Promise<boolean> {
    ...
    if (
      userAccess.customPermissions &&
      userAccess.customPermissions.includes(checkDto.permission)
    ) {
      return true;
    }
  ...
  async checkMultiplePermissions(
    checkDto: CheckMultiplePermissionsDto,
  ): Promise<{
    hasAccess: boolean;
  ...
  async assignPermissions(
    ...
    const newPermissions = [
      ...new Set([...existingPermissions, ...assignDto.permissions]),
    ];
    ...
    this.logger.log(
      `Permissions assigned to user ${assignDto.userId}: ${assignDto.permissions.join(', ')}`,
    );
  }
  ...
  async revokePermissions(
    revokeDto: RevokePermissionsDto,
    ...
    this.logger.log(
      `Permissions revoked from user ${revokeDto.userId}: ${revokeDto.permissions.join(', ')}`,
    );
  }
  ...
  async createAccessLog(createLogDto: CreateAccessLogDto): Promise<AccessLog> {
    const logData = {
      ...
      organizationId: createLogDto.organizationId
        ? new Types.ObjectId(createLogDto.organizationId)
        : undefined,
      clinicId: createLogDto.clinicId
        ? new Types.ObjectId(createLogDto.clinicId)
        : undefined,
      riskLevel: createLogDto.riskLevel || 'low',
      flaggedForReview:
        (createLogDto.riskScore && createLogDto.riskScore > 70) || false,
    };
  ...
  async getAccessLogs(query: AccessLogSearchDto): Promise<{
    ...
    if (flaggedForReview !== undefined)
      filter.flaggedForReview = flaggedForReview;
    if (organizationId)
  ...
  async bulkUserAccessAction(
    ...
    const { userIds, action, scopeType, scopeId, role, permissions, reason } =
      bulkDto;
    let success = 0;
    let failed = 0;
    const errors: string[] = [];
        ...
        switch (action) {
          case 'grant':
            if (!scopeType || !scopeId || !role) {
              throw new BadRequestException({
                message: ERROR_MESSAGES.SCOPE_PARAMETERS_REQUIRED,
                code: 'SCOPE_PARAMETERS_REQUIRED',
              });
            }
          ...
          case 'revoke':
            const accessRecords = await this.userAccessModel.find({
              userId: new Types.ObjectId(userId),
            });
            for (const access of accessRecords) {
              await this.deleteUserAccess(
                (access as any)._id.toString(),
                actionByUserId,
              );
            }
            break;
          ...
          case 'update_role':
            if (!role) {
              throw new BadRequestException({
                message: ERROR_MESSAGES.ROLE_REQUIRED,
                code: 'ROLE_REQUIRED',
              });
            }
  ...
  async setupUserAccessForOnboarding(
    userId: string,
    planType: string,
    entities: any,
  ): Promise<void> {
  ...
  async validateUserCanAccessEntity(
    userId: string,
    entityType: string,
    entityId: string,
  ): Promise<boolean> {
  ...
  private async validateScopeEntity(
    ...
    if (!entity) {
      throw new NotFoundException({
        message: ERROR_MESSAGES.SCOPE_ENTITY_NOT_FOUND,
