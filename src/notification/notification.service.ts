...
export class NotificationService {
  ...
  async create(createDto: CreateNotificationDto): Promise<Notification> {
    const notification = new this.notificationModel({
      ...
      relatedEntityId: createDto.relatedEntityId ? new Types.ObjectId(createDto.relatedEntityId) : undefined,
      deliveryStatus: createDto.deliveryMethod === 'in_app' ? 'delivered' : 'pending',
    });
  ...
  private async sendEmail(dto: CreateNotificationDto) {
    this.logger.log(`[Email Placeholder] Sending email to recipient ${dto.recipientId}: ${dto.title}`);
  ...
  private async sendSms(dto: CreateNotificationDto) {
    this.logger.log(`[SMS Placeholder] Sending SMS to recipient ${dto.recipientId}: ${dto.message}`);
