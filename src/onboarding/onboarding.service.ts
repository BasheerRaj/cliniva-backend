...
export class OnboardingService {
  ...
  private async resolvePlanId(
    ...
    if (!plan) {
      throw new BadRequestException(
        `Subscription plan not found for type: ${planType}`,
      );
    }
  ...
  async validateOnboardingData(
    ...
    const planConfig = PlanConfigUtil.getPlanConfiguration(
      onboardingDto.subscriptionData.planType,
    );
    if (!planConfig) {
    ...
    if (
      !EntityRelationshipUtil.validateEntityHierarchy(
        onboardingDto.subscriptionData.planType,
        entityHierarchy,
      )
    ) {
    ...
    const limitsValidation = PlanConfigUtil.validatePlanLimits(
      onboardingDto.subscriptionData.planType,
      entityCounts,
    );
    ...
    if (onboardingDto.workingHours && onboardingDto.workingHours.length > 0) {
      const workingHoursValidation = ValidationUtil.validateWorkingHours(
        onboardingDto.workingHours,
      );
      ...
      const hierarchicalValidation = this.validateWorkingHoursStructure(
        onboardingDto.workingHours,
      );
  ...
  private async validatePlanSpecificData(
    onboardingDto: CompleteOnboardingDto,
    errors: string[],
  ): Promise<void> {
    ...
    if (onboardingDto.organization?.legalInfo?.vatNumber) {
      if (
        !ValidationUtil.validateVATNumber(
          onboardingDto.organization.legalInfo.vatNumber,
        )
      ) {
    ...
    if (planType === 'clinic' && onboardingDto.clinics) {
      for (const clinic of onboardingDto.clinics) {
        if (clinic.businessProfile) {
          const profileValidation = ValidationUtil.validateBusinessProfile(
            clinic.businessProfile,
          );
        ...
        if (
          ...
          !ValidationUtil.validateVATNumber(clinic.legalInfo.vatNumber)
        ) {
          errors.push('Invalid VAT number format');
        }
        if (
          clinic.legalInfo?.crNumber &&
          !ValidationUtil.validateCRNumber(clinic.legalInfo.crNumber)
        ) {
    ...
    switch (planType) {
      ...
      case 'complex':
        ...
        if (
          !onboardingDto.departments ||
          onboardingDto.departments.length === 0
        ) {
  ...
  private async createEntitiesByPlan(
    ...
    switch (planType.toLowerCase()) {
      case 'company':
        entities.organization = await this.createCompanyPlanEntities(
          subscriptionId,
          onboardingDto,
          userId,
        );
      ...
      case 'complex':
        const complexEntities = await this.createComplexPlanEntities(
          subscriptionId,
          onboardingDto,
        );
      ...
      case 'clinic':
        entities.clinics = await this.createClinicPlanEntities(
          subscriptionId,
          onboardingDto,
        );
        break;
      ...
      for (const serviceData of onboardingDto.services) {
        ...
        if (planType === 'clinic' && entities.clinics?.length > 0) {
          ...
          serviceDto = {
            ...serviceData,
            clinicId:
              entities.clinics[0].id || entities.clinics[0]._id?.toString(),
          ...
          console.warn(
            `Skipping service ${serviceData.name} - no valid parent entity`,
          );
          continue;
        }
        ...
        try {
          const createdService =
            await this.serviceService.createService(serviceDto);
  ...
  private async createCompanyPlanEntities(
    ...
    if (!organizationData?.name) {
      throw new BadRequestException(
        'Organization name is required for company plan',
      );
    }
    ...
    if (onboardingDto.complexes && onboardingDto.complexes.length > 0) {
      for (const complexData of onboardingDto.complexes) {
        ...
        if (complexData.departmentIds && complexData.departmentIds.length > 0) {
          await this.createDepartmentsForComplex(
            (complex as any).id || (complex as any)._id?.toString() || '',
            complexData.departmentIds,
          );
        }
      }
    }
    ...
    if (onboardingDto.clinics && onboardingDto.clinics.length > 0) {
      for (const clinicData of onboardingDto.clinics) {
        ...
        const clinic =
          await this.clinicService.createClinic(flattenedClinicData);
  ...
  private async createSupportingEntities(
    onboardingDto: CompleteOnboardingDto,
    entities: any,
    userId: string,
  ): Promise<void> {
    ...
    if (onboardingDto.workingHours && onboardingDto.workingHours.length > 0) {
      ...
      const validation = this.validateWorkingHoursStructure(
        onboardingDto.workingHours,
      );
      if (!validation.isValid) {
        throw new BadRequestException(
          `Working hours validation failed: ${validation.errors.join(', ')}`,
        );
      }

      // Create working hours with hierarchical validation
      await this.createHierarchicalWorkingHours(
        onboardingDto.workingHours,
        entities,
      );
    }
    ...
    if (onboardingDto.contacts && onboardingDto.contacts.length > 0) {
      ...
      await this.contactService.createBulkContacts(
        onboardingDto.contacts,
        entityMappings,
      );
    }
    ...
    if (onboardingDto.legalInfo) {
      const entityMappings = this.buildEntityMappings(entities);
      await this.dynamicInfoService.createLegalDocuments(
        onboardingDto.legalInfo,
        entityMappings,
      );
    }
  }

  private buildEntityMappings(
    entities: any,
    ...
    if (entities.organization) {
      mappings.push({
        ...
        id:
          ...
          entities.organization._id?.toString() ||
          '',
      });
    }
  ...
  private validateWorkingHoursStructure(workingHours: any[]): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    for (const [day, complexDay] of complexHours) {
      ...
      if (orgDay && complexDay.isWorkingDay && !orgDay.isWorkingDay) {
        errors.push(
          `Complex cannot be open on ${day} when Organization is closed`,
        );
      }
      if (orgDay && complexDay.isWorkingDay && orgDay.isWorkingDay) {
  ...
  private async createHierarchicalWorkingHours(
    workingHours: any[],
    entities: any,
  ): Promise<void> {
    const entityMappings = this.buildEntityMappingsWithHierarchy(entities);
    ...
    for (const wh of workingHours) {
      ...
      const entityMapping = entityMappings.find(
        (m) => m.name === wh.entityName && m.type === wh.entityType,
      );
      ...
      if (entityMapping) {
        ...
        console.warn(
          `Could not find entity mapping for ${wh.entityType}: ${wh.entityName}`,
        );
      }
    }
    ...
    for (const [key, schedule] of workingHoursByEntity) {
      ...
      try {
        ...
        console.error(
          `Failed to create working hours for ${entityType} ${entityId}:`,
          error,
        );
  ...
  async saveComplexSchedule(
    ...
    try {
      ...
      await this.workingHoursService.updateWorkingHours(
        'complex',
        complexId,
  ...
  private async calculateCurrentProgress(
    ...
    if (existingOrg) {
      ...
      if (planType === 'company') {
        ...
        if (existingComplex) {
          progress.entityIds.complexId = (
            existingComplex._id as any
          ).toString();
          progress.completedSteps.push(
            'complex-overview',
            'complex-contact',
          ...
          if (existingClinic) {
            progress.entityIds.clinicId = (
              existingClinic._id as any
            ).toString();
            progress.completedSteps.push(
              'clinic-overview',
              'clinic-contact',
