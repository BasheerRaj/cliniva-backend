...
describe('ValidationUtil', () => {
  ...
  describe('validateVATNumber', () => {
    it('should validate Saudi Arabia VAT number format', () => {
      expect(ValidationUtil.validateVATNumber('300123456789001', 'SA')).toBe(
        true,
      );
      expect(ValidationUtil.validateVATNumber('123456789012345', 'SA')).toBe(
        true,
      );
    });

    it('should reject invalid Saudi Arabia VAT number format', () => {
      expect(ValidationUtil.validateVATNumber('30012345678900', 'SA')).toBe(
        false,
      ); // 14 digits
      expect(ValidationUtil.validateVATNumber('3001234567890011', 'SA')).toBe(
        false,
      ); // 16 digits
      expect(ValidationUtil.validateVATNumber('30012345678900A', 'SA')).toBe(
        false,
      ); // Contains letter
    });
    ...
    it('should return true for empty VAT number (optional field)', () => {
      ...
      expect(ValidationUtil.validateVATNumber(undefined as any, 'SA')).toBe(
        true,
      );
    });
  });
  ...
  describe('validateWorkingHours', () => {
    ...
    it('should require opening and closing times for working days', () => {
      ...
      expect(result.errors).toContain(
        'Opening and closing times required for working day: monday',
      );
    });
    ...
    it('should validate time format', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Invalid opening time format for monday: 25:00',
      );
      expect(result.errors).toContain(
        'Invalid closing time format for monday: 17:60',
      );
    });
    ...
    it('should validate break time format', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Invalid break start time format for monday: 25:00',
      );
      expect(result.errors).toContain(
        'Invalid break end time format for monday: 13:60',
      );
    });
  ...
  describe('validateSocialMediaUrl', () => {
    ...
    it('should validate WhatsApp URLs', () => {
      ...
      expect(
        ValidationUtil.validateSocialMediaUrl(
          'whatsapp',
          'https://whatsapp.com/966501234567',
        ),
      ).toBe(true);
    });
    ...
    it('should reject invalid social media URLs', () => {
      ...
      expect(
        ValidationUtil.validateSocialMediaUrl('whatsapp', 'invalid-url'),
      ).toBe(false);
    });

    it('should use generic validation for unknown platforms', () => {
      expect(
        ValidationUtil.validateSocialMediaUrl('unknown', 'https://example.com'),
      ).toBe(true);
      expect(
        ValidationUtil.validateSocialMediaUrl('unknown', 'invalid-url'),
      ).toBe(false);
    });
    ...
    it('should return true for empty URLs (optional field)', () => {
      ...
      expect(
        ValidationUtil.validateSocialMediaUrl('instagram', undefined as any),
      ).toBe(true);
    });
  });
  ...
  describe('validateBusinessProfile', () => {
    ...
    it('should validate vision statement length', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Vision statement cannot exceed 1000 characters',
      );
    });
  ...
  describe('validateEmail', () => {
    it('should validate correct email formats', () => {
      ...
      expect(ValidationUtil.validateEmail('user.name+tag@domain.co.uk')).toBe(
        true,
      );
  ...
  describe('validateGoogleLocation', () => {
    it('should validate coordinate format', () => {
      expect(ValidationUtil.validateGoogleLocation('24.7136,46.6753')).toBe(
        true,
      );
      expect(ValidationUtil.validateGoogleLocation('-34.6037,58.3816')).toBe(
        true,
      );
    ...
    it('should validate Google Place ID format', () => {
      expect(
        ValidationUtil.validateGoogleLocation('ChIJN1t_tDeuEmsRUsoyG83frY4'),
      ).toBe(true);
      expect(
        ValidationUtil.validateGoogleLocation('ChIJrTLr-GyuEmsRBfy61i59si0'),
      ).toBe(true);
    });
    ...
    it('should validate address format', () => {
      ...
      expect(
        ValidationUtil.validateGoogleLocation('123 Main Street, City, Country'),
      ).toBe(true);
    });
  ...
  describe('parseTime and formatTimeFromMinutes', () => {
    it('should parse time correctly', () => {
      ...
      expect(ValidationUtil.isTimeWithinRange('10:30', '09:00', '17:00')).toBe(
        true,
      );
      expect(ValidationUtil.isTimeWithinRange('08:30', '09:00', '17:00')).toBe(
        false,
      );
      expect(ValidationUtil.isTimeWithinRange('18:30', '09:00', '17:00')).toBe(
        false,
      );
    });
